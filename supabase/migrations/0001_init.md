...
把下面代码块里的全部内容复制，粘贴到 Supabase 控制台 → **SQL Editor** → New query，然后点 **Run** 即可一次性建好所有表、视图和权限策略。
...
- 8 张表：`competitions`、`seasons`、`teams`、`players`、`matches`、`match_lineups`、`predictions`、`sync_log`
- 1 个视图：`player_win_rates`（球员胜率统计的数据源）
- 所有表已开启 RLS，并配置好读写策略

如果提示某张表已存在也没关系，`if not exists` / `or replace` 会自动跳过或覆盖，不会报错中断。
